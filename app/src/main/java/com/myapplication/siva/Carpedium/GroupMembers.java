...
String EventNo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (regino != null) {
            ...
            sqliteDB.insertUser(queryValues);
        ...
        if(results ==null)
        { //Log.d("use",results);
        userList = sqliteDB.getAllGroupUsers(EventNo,GroupName);
        }
        else
        {Log.d("RES",results);
        ...
        if (userList.size() != 0) {
            ...
            Toast.makeText(getApplicationContext(), sqliteDB.getSyncStatus(), Toast.LENGTH_LONG).show();
        ...
        Memlist.setOnItemClickListener(new AdapterView.OnItemClickListener() {
           ...
           ////here app cant  create dialogs!!
                ...
                AlertDialog.Builder builder2 = new AlertDialog.Builder(GroupMembers.this);
                builder2.setTitle("Choose Position");


                final EditText input = new EditText(GroupMembers.this);
...
//dismissing the dialog when the user makes a selection.
                        ...
                        final AlertDialog.Builder builder3 = new AlertDialog.Builder(GroupMembers.this);
                        builder3.setTitle("ENTER PASSWORD");

                        final EditText input1 = new EditText(GroupMembers.this);
                        // Specify the type of input expected; this, for example, sets the input as a password, and will mask the text
                        input1.setInputType(InputType.TYPE_TEXT_VARIATION_PASSWORD);
                        input1.setTextColor(Color.BLACK);
                        builder3.setView(input1);
                        final TextView text = new TextView(GroupMembers.this);
                        text.setText("");
                        builder3.setView(input1);
...
User.put("userName",userList.get(position).get("RegiNo"));
                                    ...
                                    adapter = new SimpleAdapter(GroupMembers.this, userList, R.layout.swipeviewrowitem, new String[]{"Rank", "GroupName"}, new int[]{R.id.use, R.id.mainText});
